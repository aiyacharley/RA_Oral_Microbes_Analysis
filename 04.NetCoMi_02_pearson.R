...
ana.HCV1$graphlet$gcm1
ana.HCV1$graphlet$gcm2
...
plot(ana.HCV1,groupNames = c('HC','V1'), 
     ...
     edgeFilter = "threshold", edgeFilterPar = 0.5,
     nodeSize = "mclr", 
     nodeSizeSpread = 2, 
     nodeTransp = 0,
...
write.csv(gcmtest$sigDiff, file = paste0(outdir,"/GCMs_HCV1_sigDiff.csv"),row.names = T)
write.csv(gcmtest$pAdjustDiff, file = paste0(outdir,"/GCMs_HCV1_pAdjustDiff.csv"),row.names = T)
...
gcmtest <- testGCM(gobj1, gobj2, verbose = FALSE)
write.csv(gcmtest$sigDiff, file = paste0(outdir,"/GCMs_V1V4_sigDiff.csv"),row.names = T)
write.csv(gcmtest$pAdjustDiff, file = paste0(outdir,"/GCMs_V1V4_pAdjustDiff.csv"),row.names = T)
...
# 定义断点（注意：需要包含最小值0和最大值+1确保全覆盖）
breaks <- c(-Inf, 0, 5, 10, 15, 20, 25, 30, 35, 40, Inf)
...
# 创建分箱变量
merged_df$kcore_group <- cut(merged_df$kcore, 
                             breaks = breaks, 
                             labels = labels,
                             right = TRUE,      # 右闭区间（默认）
                             include.lowest = FALSE)  # 不包括最小值端点

merged_df$kcore_group <- factor(merged_df$kcore_group, levels = labels)

fig_kcore <- ggplot(merged_df, aes(x = kcore_group, y = percentage, fill = group)) +
...
# Zi-Pi plot
abs_r <- 0
res.HC <- plot_zipi(ps = ps.HC, net = net.HC, abs_r = abs_r, title="HC")
res.V1 <- plot_zipi(ps = ps.V1, net = net.V1, abs_r = abs_r, title="V1")
res.V1p <- plot_zipi(ps = ps.V1p, net = net.V1p, abs_r = abs_r, title="V1 (paired)")
res.V4 <- plot_zipi(ps = ps.V4, net = net.V4, abs_r = abs_r, title="V4")
...
# correlation
library(RColorBrewer)
col1 <- colorRampPalette(c("#E71B25", "#F8F8F8", "#4477AA"))(200)
#col1 <- colorRampPalette(c("#E71B25", "#F8F8F8", "#006300"))(200)

corr_cut <- 0
pdf(file = paste0(outdir,"/correlation",corr_cut,".pdf"), width = 16, height = 15)
tmp <- net.HC$assoMat1[nodeMAGs,nodeMAGs]
tmp[abs(tmp)<corr_cut] <- 0
fig_corr_HC <- corrplot::corrplot(tmp, 
                                  col = col1,
                                  order = c("original", "AOE", "FPC", "hclust", "alphabet")[4],
                                  type = "lower",diag = F,
                                  tl.pos = "l",tl.col = "black",title = "HC")
write.csv(fig_corr_HC$corr, file = paste0(outdir,"/correlation",corr_cut,"_HC.csv"),row.names = T)

tmp <- net.V1$assoMat1[nodeMAGs,nodeMAGs]
tmp[abs(tmp)<corr_cut] <- 0
fig_corr_V1 <- corrplot::corrplot(tmp, 
                                  col = col1,
                                  order = c("original", "AOE", "FPC", "hclust", "alphabet")[4],
                                  type = "lower",diag = F,
                                  tl.pos = "l",tl.col = "black",title = "V1")
write.csv(fig_corr_V1$corr, file = paste0(outdir,"/correlation",corr_cut,"_V1.csv"),row.names = T)

tmp <- net.V4$assoMat1[nodeMAGs,nodeMAGs]
tmp[abs(tmp)<corr_cut] <- 0
fig_corr_V4 <- corrplot::corrplot(tmp, 
                                   col = col1,
                                   order = c("original", "AOE", "FPC", "hclust", "alphabet")[4],
                                   type = "lower",diag = F,
                                   tl.pos = "l",tl.col = "black",title = "V4")
write.csv(fig_corr_V4$corr, file = paste0(outdir,"/correlation",corr_cut,"_V4.csv"),row.names = T)
dev.off()
