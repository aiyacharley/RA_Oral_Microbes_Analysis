...
write.csv(diff_set@data, file = paste0(outdir,"/venn_HCV1.csv"), row.names = F)
### 至少两种方法检测出差异的菌
...
diff_set <- ggvenn::ggvenn(lists, fill_color = c('#E69F00','#2CA02C','#1F77B4'), stroke_size = 0.5, set_name_size = 4)
diff_set
ggsave(filename = paste0(outdir,"/Fig7B_venn_HC.pdf"), diff_set, width = 5.5, height = 5.5)
diff_set@data$Hit <- diff_set@data$MaAsLin3+diff_set@data$`ANCOMBC-2`+diff_set@data$LEfSe
write.csv(diff_set@data, file = paste0(outdir,"/Fig7B_venn_HC.csv"), row.names = F)
### 至少两种方法检测出差异的菌
#diff_set2 <- subset(diff_set@data, Hit>=2)
### 三种方法并集
diff_set2 <- diff_set@data
...
diff_set <- ggvenn::ggvenn(lists, fill_color = c('#E69F00','#2CA02C','#1F77B4'), stroke_size = 0.5, set_name_size = 4)
diff_set
ggsave(filename = paste0(outdir,"/Fig7C_venn_V1.pdf"), diff_set, width = 5.5, height = 5.5)
diff_set@data$Hit <- diff_set@data$MaAsLin3+diff_set@data$`ANCOMBC-2`+diff_set@data$LEfSe
write.csv(diff_set@data, file = paste0(outdir,"/Fig7C_venn_V1.csv"), row.names = F)
### 至少两种方法检测出差异的菌
#diff_set2 <- subset(diff_set@data, Hit>=2)
### 三种方法并集
diff_set2 <- diff_set@data
...
### 对于差异KO，用MicrobiomeProfiler做富集分析（enrichKO, enrichModule）
library(MicrobiomeProfiler)
library(clusterProfiler)

outdir <- "output/Figure7/humann3ko/"
load(file = paste0(outdir,"/diff_bacteria_HCV1.rdata"))

#### HC
listsHC <- list(MaAsLin3=diff_maaslin3_HC$feature, `ANCOMBC-2`=diff_ancombc2_HC$taxon, LEfSe=diff_lefse_HC$Species)
diff_setHC <- ggvenn::ggvenn(listsHC, fill_color = c('#E69F00','#2CA02C','#1F77B4'), stroke_size = 0.5, set_name_size = 4)
diff_setHC@data$Hit <- diff_setHC@data$MaAsLin3+diff_setHC@data$`ANCOMBC-2`+diff_setHC@data$LEfSe

diff_setHC_select <- subset(diff_setHC@data, Hit>=2)
resKO <- enrichMicrobiome(gene = diff_setHC_select$`_key`, 
                          type = 'KO',
                          showCategory = 20,
                          pAdjustMethod = "BH",
                          orderBy = c('qvalue','pvalue')[1],
                          universe = NULL)
resModule <- enrichMicrobiome(gene = diff_setHC_select$`_key`, 
                              type = 'Module',
                              showCategory = 20,
                              pAdjustMethod = "BH",
                              orderBy = c('qvalue','pvalue')[1],
                              universe = NULL)
write.csv(resKO$obj@result, file = paste0(outdir,"/enrichKO_HC.csv"), row.names = F)
write.csv(resModule$obj@result, file = paste0(outdir,"/enrichModule_HC.csv"), row.names = F)

#### V1
listsV1 <- list(MaAsLin3=diff_maaslin3_V1$feature, `ANCOMBC-2`=diff_ancombc2_V1$taxon, LEfSe=diff_lefse_V1$Species)
diff_setV1 <- ggvenn::ggvenn(listsV1, fill_color = c('#E69F00','#2CA02C','#1F77B4'), stroke_size = 0.5, set_name_size = 4)
diff_setV1@data$Hit <- diff_setV1@data$MaAsLin3+diff_setV1@data$`ANCOMBC-2`+diff_setV1@data$LEfSe

diff_setV1_select <- subset(diff_setV1@data, Hit>=2)
resKO <- enrichMicrobiome(gene = diff_setV1_select$`_key`, 
                          type = 'KO',
                          showCategory = 20,
                          pAdjustMethod = "BH",
                          orderBy = c('qvalue','pvalue')[1],
                          universe = NULL)
resModule <- enrichMicrobiome(gene = diff_setV1_select$`_key`, 
                              type = 'Module',
                              showCategory = 20,
                              pAdjustMethod = "BH",
                              orderBy = c('qvalue','pvalue')[1],
                              universe = NULL)
resKO$figure
write.csv(resKO$obj@result, file = paste0(outdir,"/enrichKO_V1.csv"), row.names = F)
write.csv(resModule$obj@result, file = paste0(outdir,"/enrichModule_V1.csv"), row.names = F)
